...
deleteNodeRecursive(current, value) {
    if (!current) {
        return null;
    }
    if (current.value === value) {
        if (!current.left && !current.right) {
            return null; // Node to be deleted is a leaf node
            ...
            current.right = this.deleteNodeRecursive(current.right, successor.value);
            return current;
        }
    }
    current.left = this.deleteNodeRecursive(current.left, value);
    current.right = this.deleteNodeRecursive(current.right, value);
    return current;
}
...
// console.log(tr.findSecondSmallest()) // output: 1
